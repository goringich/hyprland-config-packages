...
if [ -r /etc/os-release ]; then
  . /etc/os-release || true
  if echo "$NAME $ID" | grep -qi cachy; then
    OS_NAME="CachyOS"
  elif echo "$NAME $ID" | grep -qi arch; then
    OS_NAME="Arch"
  fi
fi

# Colors
c_rst="\033[0m"; c_b="\033[1m"; c_blue="\033[34m"; c_cyan="\033[36m"; c_mag="\033[35m"; c_dim="\033[2m"

read -r KERNEL < <(uname -r)
read -r HOST < <(uname -n)
UPTIME=$(uptime -p | sed 's/^up //')
LOAD=$(awk '{print $1" " $2" " $3}' /proc/loadavg)
MEM=$(free -m | awk '/Mem:/ {printf "%d/%dMB", $3, $2}')
CPU=$(grep -m1 'model name' /proc/cpuinfo | sed 's/.*: //')
CPU_SHORT=$(echo "$CPU" | sed 's/\(\w\+\).*@.*/\1/' )
